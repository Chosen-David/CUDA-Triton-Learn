...
void warp4Smem(const half *A, const half *B, half *C, size_t N, size_t K, cudaStream_t stream) {
    static size_t cached_K = 0;
    static size_t smem_max_size = 0;
    if (cached_K != K) {
        smem_max_size = initWarp4Smem(K);
        cached_K = K;
    }
    ...
    warp4SmemKernel<<<grid, block, smem_max_size, stream>>>(A, B, C, N, K);
}
